...
FBconn = firebase.FirebaseApplication('https://ex-5-cloud-default-rtdb.firebaseio.com/',None)
while True:
...
firebase = firebase.FirebaseApplication('https://ex-5-cloud-default-rtdb.firebaseio.com/', None)
...
def update_temperature(record_id, new_countWord):
    """Update an existing temperature record"""
    return FBconn.put('/myTest1/' + record_id, 'count', new_countWord)

def delete_record(record_id):
    """Delete a word count record"""
    return FBconn.delete('/myTest1/', record_id)

def add_single_word(singleWord):
    """Display all temperature records"""
    records = get_all_records()
    wordExits = False
    if records:
        print("\nCurrent Records:")
        for record_id, data in records.items():
            if (data['word'] == singleWord):
                wordExits = True
                newCount = int(data['count']) + 1
                FBconn.put('/myTest1/' + record_id, 'count', newCount)
    else:
        print("\nNo records found")
    if (wordExits == False):
        data_to_upload = {
        ...
        result = FBconn.post('/myTest1/', data_to_upload)
        print(f"Word added successfully! Record ID: {result['name']}")
...
while True:
    ...
    if choice == '1':
        ...
        try:
          add_text(input("What is the text? "))
        except ValueError:
        ...
        try:
            new_count = int(input("Enter the new word count: "))
            result = update_temperature(record_id, new_count)
            if result:
                print("Word count updated successfully!")
            else:
                print("Failed to update Word count. Check if the ID exists.")
        except ValueError:
